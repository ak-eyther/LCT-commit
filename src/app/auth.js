...
async function checkAuth() {
    const token = sessionStorage.getItem('lctAuthToken');
    if (!token) {
    ...
    try {
        ...
        if (!response.ok) {
            sessionStorage.removeItem('lctAuthToken');
            sessionStorage.removeItem('lctUser');
            sessionStorage.removeItem('lctRememberMe');
            window.location.href = '/login.html';
        }
    } catch (error) {
...
function getCurrentUser() {
    try {
        const userStr = sessionStorage.getItem('lctUser');
        ...
        if (!user || !user.email || !user.id) {
            console.warn('Corrupted user data in sessionStorage');
            sessionStorage.removeItem('lctUser');
            return null;
        }
...
function isLoggedIn() {
    return !!sessionStorage.getItem('lctAuthToken');
}
