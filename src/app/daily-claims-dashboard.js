...
function enhanceClaims(rawClaims) {
  return rawClaims.map(claim => {
    ...
    const variance =
      claim.totalRequestAmt > 0
        ? Math.abs(claim.totalExtractedAmt - claim.totalRequestAmt) /
          claim.totalRequestAmt
        : 0;
    return {
      ...claim,
      ...provider,
...
function calculateMetrics(rawClaims) {
  ...
  const successfulExtractions = enhancedClaims.filter(
    claim => claim.dataExtracted === 'Yes'
  ).length;
  const adjudicated = enhancedClaims.filter(
    claim => claim.dataExtracted === 'Yes' && claim.adjudicated === 'Yes'
  ).length;
  const failedExtractions = enhancedClaims.length - successfulExtractions;
  const pendingAdjudication = enhancedClaims.filter(
    claim => claim.dataExtracted === 'Yes' && claim.adjudicated === 'No'
  ).length;
  ...
  const adjudicationRate =
    successfulExtractions > 0 ? (adjudicated / successfulExtractions) * 100 : 0;
  ...
  const providerStats = Array.from(providerStatsMap.values()).map(stat => {
    const relatedClaims = enhancedClaims.filter(
      claim => claim.providerCode === stat.providerCode
    );
    const extracted = relatedClaims.filter(
      claim => claim.dataExtracted === 'Yes'
    ).length;
    const adjudicatedCount = relatedClaims.filter(
      claim => claim.dataExtracted === 'Yes' && claim.adjudicated === 'Yes'
    ).length;
    ...
    return {
      ...
      avgSavingsPercent:
        stat.totalRequested > 0
          ? (stat.totalSavings / stat.totalRequested) * 100
          : 0,
      extractionSuccessRate:
        relatedClaims.length > 0 ? (extracted / relatedClaims.length) * 100 : 0,
      adjudicationRate:
        extracted > 0 ? (adjudicatedCount / extracted) * 100 : 0,
      rejectionRate:
        stat.visitCount > 0
          ? (stat.fullyRejectedCount / stat.visitCount) * 100
          : 0,
    };
  });
  ...
  const rejectionRate =
    deduplicated.length > 0
      ? (totals.fullyRejected / deduplicated.length) * 100
      : 0;
  if (rejectionRate > 30) {
    alerts.push({
      id: 'rejection',
...
function renderProgressRings(metrics) {
  ...
  extractionRing.setAttribute(
    'data-label',
    formatPercent(metrics.extractionRate)
  );

  adjudicationRing.style.setProperty('--value', metrics.adjudicationRate);
  adjudicationRing.setAttribute(
    'data-label',
    formatPercent(metrics.adjudicationRate)
  );

  document.getElementById('failed-extractions').textContent =
    metrics.failedExtractions.toLocaleString('en-IN');
  document.getElementById('pending-adjudication').textContent =
    metrics.pendingAdjudication.toLocaleString('en-IN');
}
...
function renderSavingsDistribution(metrics) {
  ...
  const total =
    Object.values(metrics.savingsDistribution).reduce(
      (acc, value) => acc + value,
      0
    ) || 1;
  const slices = [
...
function renderProviderPerformance(metrics) {
  ...
  if (!stats.length) {
    container.innerHTML =
      '<p class="subtle-text">No provider data available for the selected period.</p>';
    return;
  }
...
function renderAlerts(metrics) {
  ...
  container.innerHTML = metrics.alerts
    .map(
      alert => `
        <div class="alert-row">
...
function renderClaimsTable(metrics, state) {
  ...
  document.getElementById('claims-count-summary').textContent =
...
function initExport(state) {
  document.getElementById('export-btn').addEventListener('click', () => {
    ...
    const rows = state.currentMetrics.deduplicated.map(claim =>
      headers.map(key => claim[key] ?? '').join(',')
    );
...
function initUploadModal(historyState) {
  ...
  const openButtons = document.querySelectorAll(
    '#open-upload-modal, button[data-trigger-upload]'
  );
  ...
  startButton.addEventListener('click', () => {
    ...
    // DEMO MODE: Using Math.random() for mock upload simulation only
    // TODO: Replace with real file upload and processing when backend API is ready
    ...
    intervalId = setInterval(() => {
      ...
      if (progressValue >= 100) {
        ...
        // DEMO MODE: Mock statistics (replace with real API response)
        const mockClaimsCount = Math.floor(Math.random() * 30) + 12;
        const mockSavings = Math.floor(Math.random() * 6e6);
        ...
        summary.innerHTML = `
                    ...
                    <div style="display: grid; gap: var(--spacing-2); margin-top: var(--spacing-3);">
                        <div><strong>Claims processed:</strong> ${mockClaimsCount}</div>
                        <div><strong>Total savings:</strong> ${formatIndianAmount(mockSavings)}</div>
                    </div>
                `;
        historyState.unshift({
          id: Date.now(),
          filename: uploadFile.name,
          claimsCount: mockClaimsCount,
